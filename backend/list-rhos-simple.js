...
// Connect to MongoDB and list all RHOs (simple version)
...
async function listAllRHOs() {
  try {
    ...
    // Find all RHOs without population first
    ...
    rhos.forEach((rho, index) => {
      ...
      console.log(`   Parent SHO ID: ${rho.parentSHO}`);
    });
    ...
    console.log(`🔍 SHO Name: ${sho?.fullName}`);
    console.log(`🔍 RHOs belonging to this SHO: ${rhos.filter(rho => rho.parentSHO?.toString() === sho?._id?.toString()).length}`);
